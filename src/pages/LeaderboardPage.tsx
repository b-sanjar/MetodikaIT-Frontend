...
function starsFor(points: number): number {
  ...
  return (
    <div className="animate-rise">
      <PageHeader
        ...
        subtitle="Ball, yulduz va nishonlar — sog‘lom raqobat vositasi"
        actions={
        ...
        <>
          ...
          <div className="stagger mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {podium.map((s, i) => (
              <Card
                key={s.id}
                className={cn('relative overflow-hidden border bg-linear-to-b p-5 text-center', PODIUM[i].card)}
              >
                <div
                  aria-hidden
                  className={cn(
